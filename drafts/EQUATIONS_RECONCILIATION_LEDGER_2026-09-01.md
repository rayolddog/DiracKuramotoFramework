...
**Totals: 17 items · 6 applied · 11 pending disposition.**
...
| **E-16** | LOW–MED | [P1] §4 (noise origin); §6.1 ladder | **P2's $\delta b_i$ conflates two sources with different temperature scalings.** §4 writes the noise as one increment from "the wave packets of the detector's other bound charges *and* the vacuum/environment field." These differ where it matters: the bound-electron contribution is thermal, with $\tau_c \sim 1/\Gamma(T)$, and *freezes out* on cooling; the zero-point contribution is broadband and temperature-independent. Given E-15's finding that whiteness is what carries the Born weights, the split is consequential — **cooling lengthens $\tau_c$**, so the mechanism should be more at risk in cryogenic detectors than in room-temperature ones, unless the vacuum floor alone keeps the noise white. §6.1's ladder is computed only for a room-temperature silicon SPAD, while §8 discusses SNSPDs, which run at 1–4 K. Nobody has checked whether the ladder survives there. | **Raised 2026-09-02, not yet dispositioned.** Recorded so it is not lost. Would want the ladder recomputed for the cryogenic detector families §8 actually names. |
