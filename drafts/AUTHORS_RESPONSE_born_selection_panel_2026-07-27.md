...
To make the reclassification unmissable (v0.4, drafted into §1), we state the argument's three distinct layers explicitly, since conflating them is precisely what drew the circularity charge from all four reviewers:
- **Settled — not ours to claim.** The measure is Born by Gleason's theorem, and the amplitude-squared *energy* is classical driven-oscillator energetics (the holographic $|E|^2$, no probabilistic content). The square is not the contested quantity; we do not derive it, and the manuscript already credits Gleason for the measure's uniqueness (abstract, §1, Table 2).
- **Claimed.** A physical *selection process* — a noise-driven competition among coupled real wave packets — that *realizes* that measure by turning deposited-energy shares into registration frequencies as a proved, rather than stipulated, fair game.
- **Open.** Deriving that process, with its conservation and covariance, from a closed microscopic model, so that P1's identification of energy-share with outcome-probability becomes a result rather than an input.

This separates what no reader should contest (the square) from what we actually assert (the dynamics) from what we concede is unproven (the microscopic grounding) — the same partition the panel, independently, and the authors both locate. It also disposes of a tempting but wrong rebuttal we explicitly do **not** make: that "the squaring is derived in complex analysis, so the circularity charge is vacuous." Gleason already establishes the square; the reviewers grant it; the contested step is the energy→probability bridge, which lives in the *Claimed*/*Open* layers, not in the square.
