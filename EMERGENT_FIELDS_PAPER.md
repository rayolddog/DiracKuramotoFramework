...
Brillouin-zone valleys without being inserted by hand; (ii) the **Sakaguchi
phase-lag $\alpha$ acts as a Haldane flux** — it generates Berry curvature and a
quantized Chern response $C=\pm 1=\mathrm{sign}(\alpha)$, with the topological
gap obeying the Haldane boundary $|\Delta| = 3\sqrt{3}\,t_2|\sin\alpha|$
numerically; sublattice pinning asymmetry independently supplies the trivial
(Semenoff) mass, so *both* known Dirac mass mechanisms are sync parameters;
...
asymmetry, and with dissipative first-order dynamics rather than precessional
spin dynamics.
...
- **Semenoff mass** — sublattice pinning asymmetry $\kappa_A\neq\kappa_B$ opens
  ...
  (computed).
- **Haldane mass** — a Sakaguchi lag $\alpha$ on next-nearest-neighbour
  couplings opens an opposite-sign gap: $C=\pm1=\mathrm{sign}(\alpha)$
  (computed by Berry-curvature integration), with the topological phase
  boundary at $|\Delta| = 3\sqrt3\,t_2|\sin\alpha|$ reproduced numerically
  ($=0.520$ at $\alpha=\pi/2$, $t_2=0.1$).
...
**Platforms where $\alpha$ is native.** (i) *Josephson-junction arrays*: the
phase lag arises from reactive (inductive/capacitive) coupling components;
honeycomb JJ arrays are standard fabrication. (ii) *Optomechanical / NEMS
lattices*: coupling delay through the optical bus sets $\alpha$ continuously.
(iii) *Spin-torque / spin-Hall nano-oscillator arrays*: dipolar and spin-wave
coupling has intrinsic, current-tunable phase delay. (iv) *Laser arrays*:
detuned evanescent coupling. In all four, $\alpha$ is not exotic — it is
usually the *nuisance parameter* of the synchronization literature. Here it is
promoted to the topological drive.
...
1. **Each mechanism is individually known.** Graphene cones, the Haldane model,
   ...
   *dictionary*: all of these structures generated jointly by the standard sync
   ...
   $C=\pm1=\mathrm{sign}(\alpha)$ identification stated and verified in the
   Kuramoto setting; and (b) the *platform claim*: this content is reachable by
   classical oscillator hardware. A referee-proofing derivation of exactly
   where the complex hopping enters the first-order dissipative linearization
   (the real part of $\sin(\Delta\theta+\alpha)$ linearizes to a trivial
   $\cos\alpha$-rescaled Laplacian; the flux lives in the antisymmetric /
   inertial / amplitude sector) is in progress and will accompany the final
   version.
