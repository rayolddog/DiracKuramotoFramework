...
parameter. To our knowledge the identification "Sakaguchi lag = Haldane flux,"
with its quantized Chern consequence, has not been stated or verified.
...
1. **Each mechanism is individually known.** Graphene cones, the Haldane model,
   ...
   attachment — all textbook. The contributions here are (a) the *dictionary*:
   all of them are generated by the standard sync parameters of one substrate,
   with the Sakaguchi-lag ⇒ Berry-flux ⇒ quantized-Chern-response chain stated
   and verified, we believe, for the first time; and (b) the *platform claim*:
   this content is reachable by classical oscillator hardware.
